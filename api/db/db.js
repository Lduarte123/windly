...
if (process.env.NEONDB_USER) {
  // Configuração para NeonDB (produção/nuvem)
  pool = new Pool({
    user: process.env.NEONDB_USER,
    host: process.env.NEONDB_HOST,
    database: process.env.NEONDB_DATABASE,
    password: process.env.NEONDB_PASSWORD,
    port: process.env.NEONDB_PORT,
    ssl: process.env.NEONDB_SSLMODE === 'require' ? { rejectUnauthorized: false } : false,
  });
} else {
